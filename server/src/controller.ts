...
export class MCController implements MCControllerInterface {
  ...
  public connect(ws: ws, req: Request): void {
    ws.on('message', (msg: string) => {
      const payload = JSON.parse(msg);
      payload.ws = ws;
      const event: MCEvent = this.eventBus.createEvent(
        payload.topic,
        payload,
        (res: any) => ws.send(res.message),
        (e: Error) => ws.send(e.message),
      );

      this.eventBus.publish(event);
    });

    ws.send('Succesfully connected');
  }
  ...
  public getUserById(req: Request, res: Response): void {
    try {
      ...
      const user: UserSchemaObject | void = this.MCFM.getOneById<
        UserSchemaObject
      >('users', Number(userId));
  ...
  public getServerDetails(req: Request, res: Response): void {
    try {
      ...
      const serverDetails: ServerDetails = this.MCSM.getServerDetails(
        Number(serverId)
      );
  ...
  public getServersByUserId(req: Request, res: Response): void {
    try {
      ...
      const {
        dict: servers
      }: EntityFile<ServerSchemaObject> = this.MCFM.getAll<ServerSchemaObject>(
        'servers'
      );
  ...
  public async createServer(req: Request, res: Response): Promise<void> {
    ...
    try {
      if (userId === undefined || !name || !runtime || !isEulaAccepted) {
        throw new Error(
          `The following query params are required: 'userId', 'name', 'runtime', 'isEulaAccepted'`
        );
      }

      const server: ServerSchemaObject = await this.MCSM.createServer(
        name,
        runtime,
        isEulaAccepted,
        userId,
  ...
  public async updateServerConfig(req: Request, res: Response): Promise<void> {
    ...
    try {
      if (typeof parseInt(serverId) !== 'number' || !config) {
        throw new Error(
          `The following query params are required: 'serverId', 'config'`
        );
      }

      const result: ServerConfig = await this.MCSM.updateServerConfig(
        serverId,
        config
      );
